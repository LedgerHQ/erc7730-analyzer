 ...
 *
 * Usage:
 *   node tools/scripts/migrate-v1-to-v2.js [options] [--file <path> | <path>]
 *
 * Accepted options:
...
function printHelp(exitCode = 0, errorMessage = null) {
  ...
  write("Usage: node tools/scripts/migrate-v1-to-v2.js [options] [--file <path> | <path>]");
  write("\nAccepted options:");
  for (const option of ACCEPTED_OPTIONS) {
    write(option);
  }
  write("\nExamples:");
  write("  node tools/scripts/migrate-v1-to-v2.js --dry-run");
  write("  node tools/scripts/migrate-v1-to-v2.js --file registry/midas/calldata-MinterVault.json");
...
/**
 * Format a file using erc7730 format command to comply with registry conventions.
 * @param {string} filePath - Path to the file to format
 */
function formatFile(filePath) {
  const linterCmd = getLinterCommand();
  if (!linterCmd) {
    verboseLog("  ⚠️  erc7730 CLI not found, skipping format", "WARN");
    return;
  }

  verboseLog(`  🎨 Formatting: ${path.relative(ROOT_DIR, filePath)}`);

  try {
    const result = spawnSync(linterCmd, ["format", filePath], {
      cwd: ROOT_DIR,
      encoding: "utf8",
      stdio: VERBOSE ? "inherit" : "pipe",
    });
    ...
    if (result.status !== 0) {
      const errorMsg = result.stderr || result.stdout || "Format command failed";
      console.warn(`  ⚠️  Format failed for ${path.relative(ROOT_DIR, filePath)}: ${errorMsg.slice(0, 200)}`);
    }
  } catch (error) {
    console.warn(`  ⚠️  Format error for ${path.relative(ROOT_DIR, filePath)}: ${error.message}`);
  }
}
...
/**
 * Extract all leaf paths from an ABI function entry's inputs.
 * Returns an array of JSON-path strings like "#.param", "#.param.component", "#.arr.[]".
 */
function extractAbiLeafPaths(abiEntry) {
  if (!abiEntry || !Array.isArray(abiEntry.inputs)) return [];
  const paths = [];

  function walk(params, prefix) {
    for (const param of params) {
      const name = param.name;
      const isArray = (param.type || "").endsWith("[]");
      const isTuple = (param.type || "").startsWith("tuple");

      if (isTuple && Array.isArray(param.components) && param.components.length > 0) {
        const nextPrefix = isArray ? `${prefix}.${name}.[]` : `${prefix}.${name}`;
        walk(param.components, nextPrefix);
      } else if (isArray) {
  ...
  walk(abiEntry.inputs, "#");
  return paths;
}

/**
 * Extract all leaf paths from EIP-712 schema types for a given primary type.
 * Returns an array of JSON-path strings like "#.field", "#.field.subfield".
 */
function extractEip712LeafPaths(schemas, primaryType) {
  ...
  function walk(typeName, prefix, visited) {
    if (!types[typeName] || visited.has(typeName)) return;
    visited.add(typeName);

    for (const field of types[typeName]) {
      const baseType = field.type.replace(/\[\]$/, "");
      const isArray = field.type.endsWith("[]");
      const isComplex = !!types[baseType];

      if (isComplex) {
        const nextPrefix = isArray ? `${prefix}.${field.name}.[]` : `${prefix}.${field.name}`;
        walk(baseType, nextPrefix, new Set(visited));
      } else if (isArray) {
...
/**
 * Fix inline tuple syntax in a function signature format key.
 * Strips any `tuple` keyword prefix from bare tuple params, ensuring
 * the linter-compatible format: (type1 name1, type2 name2) paramName.
 * Handles nested tuples recursively.
 */
function fixInlineTupleSyntax(key) {
  const match = key.match(/^([A-Za-z_]\w*)\(([\s\S]*)\)$/);
  if (!match) return key;

  const funcName = match[1];
  const paramsBody = match[2].trim();
  if (!paramsBody) return key;

  const params = splitTopLevelParams(paramsBody);
  const fixedParams = params.map(fixParamTupleSyntax);
...
function fixParamTupleSyntax(param) {
  ...
  // Strip leading "tuple" keyword if present (e.g. "tuple(...) name" -> "(...) name")
  if (trimmed.startsWith("tuple(")) {
    trimmed = trimmed.slice(5); // remove "tuple", keep "(...) ..."
  }
  ...
  if (closingIdx === -1) return trimmed;

  const inner = trimmed.slice(1, closingIdx);
  const after = trimmed.slice(closingIdx + 1);

  const innerParams = splitTopLevelParams(inner);
  const fixedInner = innerParams.map(fixParamTupleSyntax);
  return `(${fixedInner.join(", ")})${after}`;
}
...
/**
 * Recursively compute the canonical Solidity type for an ABI parameter,
 * expanding tuples to (type1,type2,...) form.
 */
...
/**
 * Compute the 4-byte hex selector for an ABI function entry.
 */
 ...
 * Matches by canonical signature, hex selector, or unique function name.
 */
 ...
 * Also extracts all leaf paths per format key from ABI/schema before they are deleted.
 * Returns { keyMapping, leafPathsPerFormat }.
 */
function transformFormatKeys(json) {
  if (!json.display?.formats) return { keyMapping: {}, leafPathsPerFormat: {} };

  const keyMapping = {};
  const leafPathsPerFormat = {};
  const isEip712 = !!json.context?.eip712;
  const isContract = !!json.context?.contract;

  // For EIP-712, use schemas to generate encodeType and extract leaf paths
  if (isEip712 && json.context.eip712.schemas) {
    ...
    if (Array.isArray(schemas)) {
      for (const oldKey of Object.keys(json.display.formats)) {
        ...
        const paths = extractEip712LeafPaths(schemas, oldKey);
        if (paths.length > 0) {
          leafPathsPerFormat[keyMapping[oldKey] || oldKey] = paths;
        }
      }
    }
  }

  // For contracts, try to build human-readable signatures from ABI and extract leaf paths
    ...
    if (abi) {
      for (const oldKey of Object.keys(json.display.formats)) {
        ...
        if (abiEntry) {
          ...
          const paths = extractAbiLeafPaths(abiEntry);
          if (paths.length > 0) {
            leafPathsPerFormat[keyMapping[oldKey] || oldKey] = paths;
          }
        }
  ...
  return { keyMapping, leafPathsPerFormat };
}
...
function migrateFile(filePath) {
  ...
  try {
    ...
    const { keyMapping, leafPathsPerFormat } = transformFormatKeys(json);
    ...
    // 5b. Fix inline tuple syntax: bare (...) -> tuple(...) in format keys
    if (json.display?.formats) {
      const tupleFix = {};
      for (const [key, format] of Object.entries(json.display.formats)) {
        const fixed = fixInlineTupleSyntax(key);
        tupleFix[fixed] = format;
        if (fixed !== key) modified = true;
      }
      json.display.formats = tupleFix;
    }
    ...
    if (json.display?.formats) {
      for (const [formatKey, format] of Object.entries(json.display.formats)) {
        ...
        if (format.fields && Array.isArray(format.fields)) {
          ...
          // Add missing ABI/schema paths that weren't in fields, required, or excluded
            ...
            for (const leafPath of allLeafPaths) {
              if (!existingPaths.has(normalizePath(leafPath))) {
                const pathLabel = leafPath
                  ...
                  .trim()
                  .replace(/\b\w/g, (c) => c.toUpperCase());
                format.fields.push({ path: leafPath, label: pathLabel, visible: "never" });
                modified = true;
              }
            }
          }

          // All fields must have visible in v2 — default to "always" if not set
          for (const field of format.fields) {
    ...
    if (modified) {
      if (!DRY_RUN) {
        ...
        // Format the migrated file to comply with registry conventions
        formatFile(filePath);
