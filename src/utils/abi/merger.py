...
class ABIMerger:
    ...
    def _get_function_signature(self, func: dict) -> str | None:
        """
    ...
    def _get_event_signature(self, event: dict) -> str | None:
        """
    ...
    def add_abi(self, abi: list[dict], chain_id: int, source_address: str | None = None) -> dict[str, int]:
        """
        ...
        return {"new_functions": new_functions, "new_events": new_events, "duplicate_functions": duplicate_functions}
...
def merge_abis_from_deployments(
    ...
    fetch_abi_func: Callable[[str, int, str], tuple[list[dict] | None, dict, bool]],
    api_key: str,
) -> tuple[list[dict] | None, dict, dict]:
    """
    ...
    for deployment in deployments:
        ...
        try:
            ...
            if abi:
                ...
                logger.info(
                    f"✓ Chain {chain_id}: {stats['new_functions']} new functions, "
                    f"{stats['duplicate_functions']} duplicates"
                )
            else:
                fetch_results[chain_id] = {"success": False, "error": "ABI fetch returned None"}
                failed_fetches += 1
                logger.warning(f"✗ Chain {chain_id}: Failed to fetch ABI")

        except Exception as e:
            fetch_results[chain_id] = {"success": False, "error": str(e)}
            failed_fetches += 1
