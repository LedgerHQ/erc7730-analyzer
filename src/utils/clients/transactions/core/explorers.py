...
class TransactionFetcherCoreExplorerMixin:
    def _fetch_coredao_transactions(
        self, contract_address: str, filter_type: str = "to"
        ...
        try:
            ...
            while page <= max_pages:
                ...
                if data.get("status") == "1" and "result" in data:
                    ...
                    logger.info(
                        f"Core DAO API page {page} returned {len(transactions)} transactions (total: {len(all_transactions)})"
                    )
                    ...
                    logger.info(
                        f"Core DAO API response on page {page}: status={data.get('status')}, message={data.get('message')}"
                    )
                    break

            if all_transactions:
                logger.info(
                    f"Core DAO API returned total of {len(all_transactions)} transactions across {page} page(s)"
                )
                ...
                logger.info("No transactions found from Core DAO API")
                return None
    ...
    def _fetch_blockscout_v2_transactions(
        self, contract_address: str, chain_id: int, filter_type: str = "to"
    ) -> list[dict[str, Any]] | None:
        """
    ...
    def get_block_by_timestamp(
        self, timestamp: int, closest: str, chain_id: int = 1, use_blockscout: bool = False
    ) -> int | None:
        """
