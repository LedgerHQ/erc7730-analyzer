...
class TransactionFetcherCoreQueryMixin:
    def _fetch_transactions_blockscout_v2(
        ...
        selector_candidates = {s: [] for s in selectors}
        selector_candidate_limit = self._tx_sample_candidate_limit(per_selector)
        ...
        payable_selectors = {s for s in payable_selectors if s in selector_candidates}
        ...
        for tx in blockscout_txs:
            ...
            if tx_selector in selector_candidates and len(selector_candidates[tx_selector]) < selector_candidate_limit:
                selector_candidates[tx_selector].append(etherscan_tx)
                logger.debug(f"Found candidate for {tx_selector}: {etherscan_tx.get('hash')}")
        ...
        for sel, tx_list in selector_txs.items():
            if sel in payable_selectors:
                ...
                logger.info(
                    f"Selector {sel} (payable): found {len(tx_list)}/{per_selector} transactions ({native_count} native ETH, {erc20_count} ERC20)"
                )
            else:
    ...
    def _fetch_transactions_with_api(
        ...
        def finalize_results() -> dict[str, list[dict[str, Any]]]:
            return self._finalize_selector_transaction_samples(
                selector_candidates,
                per_selector=per_selector,
                payable_selectors=payable_selectors,
            )
        ...
        def all_done() -> bool:
            ...
            return all(len(candidates) >= selector_candidate_limit for candidates in selector_candidates.values())
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt + 1 == max_retries:
                            ...
                            partial_results = finalize_results()
