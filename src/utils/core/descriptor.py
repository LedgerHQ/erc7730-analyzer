...
class AnalyzerDescriptorMixin:
    ...
    def _merge_includes(self, data: dict[str, Any], base_path: Path) -> dict[str, Any]:
        ...
        if self._detect_erc4626_from_includes(include_file):
            ...
            self.erc4626_context = self._build_erc4626_context(
                includes_detected=True, source_detection={}, underlying_token=underlying_token
            )
        ...
        try:
            ...
            if "display" in include_data:
                ...
                if "definitions" in include_data["display"]:
                    ...
                    data["display"]["definitions"] = {
                        **include_data["display"]["definitions"],
                        **data["display"]["definitions"],
                    }
    ...
    def _match_function_signature_to_abi(self, signature: str) -> dict[str, Any] | None:
        if not self.abi_helper or not signature or signature.startswith("0x"):
            return None

        function_data = self.abi_helper.find_function_by_signature(signature)
        return function_data or None

    def _build_function_metadata_from_format_key(
        self,
        format_key: str,
        selector: str | None = None,
    ) -> dict[str, Any] | None:
        """
        Synthesize minimal function metadata from a display.format key.

        This is primarily used for prepared benchmark inputs where decoded
        transactions are already available and the merged ABI may only contain a
        benchmark-specific subset.
        """
        if not format_key or format_key.startswith("0x"):
            return None

        signature = format_key.strip()
        if "(" not in signature:
            return None
        ...
        canonical_signature = self._normalize_function_signature(signature)
        resolved_selector = (
            selector.lower()
            if selector and selector.startswith("0x")
            else "0x" + self.w3.keccak(text=canonical_signature).hex()[:8]
        )
        ...
        return {
            ...
            "tuple_keyword_display_signature": signature,
    ...
    def extract_selectors(self, erc7730_data: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
        ...
        if "display" in erc7730_data and "formats" in erc7730_data["display"]:
            ...
            for key in formats.keys():
                ...
                abi_match = self._match_function_signature_to_abi(formatted_key)
                if abi_match:
                    selector = abi_match["selector"].lower()
    ...
    def _normalize_param_type(self, param: str) -> str:
        ...
        # Tuple parameter e.g. "(address src,address dst) desc" or
        # "tuple(address src,address dst) desc"
        tuple_start_idx = None
        if param[0] == "(":
            tuple_start_idx = 0
        elif param.startswith("tuple("):
            tuple_start_idx = len("tuple")

        if tuple_start_idx is not None:
            depth = 0
            inner_chars = []
            idx = tuple_start_idx
            ...
            normalized_inner = ",".join(filter(None, (self._normalize_param_type(p) for p in inner_params)))
        ...
        try:
            ...
            for deployment in deployments:
                ...
                if address and chain_id:
                    deployment_list.append({"address": address, "chainId": chain_id})
    ...
    def get_function_abi_by_selector(self, selector: str) -> dict | None:
        """
        ...
        fallback_metadata = self._build_function_metadata_from_format_key(
            format_key,
