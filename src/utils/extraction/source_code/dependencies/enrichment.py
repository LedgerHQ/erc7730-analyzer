...
def build_dependency_result(
    ...
    facet_specific_code: dict[str, Any] | None,
    facet_addr: str | None,
    max_lines: int,
) -> dict[str, Any]:
    ...
    if facet_specific_code:
        ...
        logger.debug("  📦 Using merged dependencies (no facet-specific source)")
    ...
    while internal_calls_to_process:
        ...
        if func_to_use:
            result["internal_functions"].append(
                {"body": func_to_use["body"], "docstring": func_to_use.get("docstring")}
            )
        ...
        if len(parts) == 2:
            ...
            for func_data in dependency_source.get("internal_functions", {}).values():
                if func_data["name"] == func_name:
                    ...
                    result["internal_functions"].append(
                        {"body": func_data["body"], "docstring": func_data.get("docstring")}
                    )
            ...
            if not found and fallback_source:
                ...
                for _func_sig, func_data in all_funcs_cache.items():
                    if func_data["name"] == func_name:
                        ...
                        result["internal_functions"].append(
                            {"body": lib_func_body, "docstring": func_data.get("docstring")}
                        )
    ...
    if super_calls and extracted_code.get("source_code"):
        ...
        if inheritance_chain:
                    ...
                    for parent_name in parents:
                        ...
                        if parent_func:
                            ...
                            for nested_call in parent_internal_calls:
                                if (
                                    nested_call not in processed_internal_calls
            ...
            logger.warning("   ⚠ No inheritance chain found in source code")
