...
class SourceCodeFetchingExtractionMixin:
    def extract_contract_code(
        ...
        selector_sources: dict[str, list[dict]] | None = None,
    ) -> dict[str, Any]:
        """
        ...
        if facets and not facets.get("_is_diamond_but_unmapped"):
            ...
            def fetch_and_parse_facet(facet_addr):
                ...
                if is_vyper:
                    ...
                    facet_result["internal_functions"] = {
                        k: v for k, v in facet_result["functions"].items() if v["visibility"] == "internal"
                    }
                else:
                    ...
                    facet_result["internal_functions"] = {
                        k: v for k, v in facet_result["functions"].items() if v["visibility"] in ["internal", "private"]
                    }
            ...
            try:
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = {executor.submit(fetch_and_parse_facet, addr): addr for addr in unique_facet_addresses}
            ...
            logger.info(
                f"Parallel extraction complete: {len(per_facet_codes)}/{len(unique_facet_addresses)} facets succeeded in {elapsed_total:.1f}s"
            )
        ...
        if is_vyper:
            ...
            result["internal_functions"] = {
                k: v for k, v in result["functions"].items() if v["visibility"] == "internal"
            }
        else:
            ...
            result["internal_functions"] = {
                k: v for k, v in result["functions"].items() if v["visibility"] in ["internal", "private"]
            }
