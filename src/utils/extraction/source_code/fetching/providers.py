...
class SourceCodeFetchingProviderMixin:
    ...
    def get_contract_name_from_etherscan(self, contract_address: str, chain_id: int) -> str | None:
        """
    ...
    def fetch_source_from_etherscan(self, contract_address: str, chain_id: int) -> str | None:
        """
    ...
    def fetch_source_from_blockscout(self, contract_address: str, chain_id: int) -> str | None:
        """
        ...
        try:
                            ...
                            if source_code:
                                ...
                                if source_code.startswith("{{"):
                                    ...
                                    if combined_code:
                                        final_source = "\n\n".join(combined_code)
                                        logger.info(
                                            f"Fetched source code from Core DAO API ({len(final_source)} chars, {len(combined_code)} files)"
                                        )
                                        return final_source
                                else:
            ...
            if not data or "source_code" not in data:
                logger.debug("No source code found on Blockscout")
                return None
            ...
            if isinstance(source_code, str):
                ...
                final_source = "\n\n".join(combined_code)
                logger.info(
                    f"Fetched source code from Blockscout ({len(final_source)} chars, {len(combined_code)} files)"
                )
                return final_source
