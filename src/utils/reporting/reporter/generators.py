...
"""
    ...
    for selector, selector_data in results.get("selectors", {}).items():
        ...
        if transactions:
            ...
            erc7730_fields = {
                field.get("path", "").replace("params.", ""): field for field in (erc7730_format.get("fields") or [])
            }
            excluded_fields = [e.replace("params.", "") for e in (erc7730_format.get("excluded") or [])]

            total_params = len(decoded_input)
            shown_count = len([p for p in decoded_input if p in erc7730_fields])
            excluded_count = len([p for p in decoded_input if p in excluded_fields])
            missing_count = total_params - shown_count - excluded_count
            coverage_pct = (shown_count / total_params * 100) if total_params > 0 else 0

            missing_params = [p for p in decoded_input if p not in erc7730_fields and p not in excluded_fields]
